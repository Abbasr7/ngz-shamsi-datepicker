...
export class JalaliMomentDateAdapter extends NzDateAdapter<Moment> {
  constructor(private i18n: NzI18nService) {
    super();
    i18n.localeChange.subscribe(res => {
      let locale = res.locale == 'fa_Gregorian' ? 'en': res.locale;
      moment.locale(locale);
      console.log(locale);
      
    })
    // moment.updateLocale('fa', { weekdaysMin: ['ی', 'د', 'س', 'چ', 'پ', 'ج', 'ش'] });
  }
  ...
  parse(text: string, formatStr: string, options: { locale: Locale; weekStartsOn: number } | undefined): Moment {
